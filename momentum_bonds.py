...
def train_production(df: pd.DataFrame, num_attribs: List[str], cat_attribs: List[str], bool_attribs: List[str]) -> None:
    ...
    current_events = df.swaplevel().sort_index().loc['2021-01-01'].drop(columns=['bin', 'ret', 'clfW', 't1'])

    pred_now = rf.predict(current_events)
    score_now = rf.predict_proba(current_events)[:,1]

    data = {"pred": pred_now, "side": current_events.side.values, "close": current_events.close.values}
    trades = pd.DataFrame(data, index=current_events.index)
    trades = trades.join(desc[['name', 'cpn', 'maturity']])
    ...
    print(trades[trades.pred == 1].loc['2021-01-01'])
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        try:
            df = df.xs(ticker, drop_level=False)
        except:
            print(ticker, 'not in price data')
            return
