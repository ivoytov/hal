...
from util.fast_ewma import ewma
from data_structures.base_bars import BaseBars
...
class RunBars(BaseBars):
    ...
    def __init__(self, ticks, metric, num_prev_bars=3, exp_num_ticks_init=100000, batch_size=2e7):
        """
        Constructor
        ...
        """
        BaseBars.__init__(self, ticks, metric, batch_size)
...
def get_dollar_run_bars(file_path, ISIN, num_prev_bars, exp_num_ticks_init=100000,
    ...
    bars = RunBars(store=file_path, ISIN=ISIN, metric='dollar_run', num_prev_bars=num_prev_bars,
...
def get_volume_run_bars(ticks, num_prev_bars, exp_num_ticks_init=100000,
                        batch_size=2e7, verbose=True, to_csv=False, output_path=None):
    """
    ...
    """
    bars = RunBars(ticks=ticks, metric='volume_run', num_prev_bars=num_prev_bars,
                   exp_num_ticks_init=exp_num_ticks_init, batch_size=batch_size)
...
def get_tick_run_bars(file_path, ISIN, num_prev_bars, exp_num_ticks_init=100000,
                      batch_size=2e7, verbose=True, to_csv=False, output_path=None):
    """
    Creates the tick run bars: date_time, open, high, low, close, volume.
    :param file_path: File path pointing to hdf data.
    :param num_prev_bars: Number of previous bars used for EWMA window expected # of ticks
    :param exp_num_ticks_init: initial expected number of ticks per bar
    :param batch_size: The number of rows per batch. Less RAM = smaller batch size.
    :param verbose: Print out batch numbers (True or False)
    ...
    """
    bars = RunBars(store=file_path, ISIN=ISIN, metric='tick_run', num_prev_bars=num_prev_bars,
                   exp_num_ticks_init=exp_num_ticks_init, batch_size=batch_size)
