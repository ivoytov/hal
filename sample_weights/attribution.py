...
from sampling.concurrent import num_concurrent_events, get_av_uniqueness_from_triple_barrier
...
def getWeightColumn(bins: pd.DataFrame, prices: pd.DataFrame):
  ...
  for ticker in bins.dropna(subset=['t1']).index.unique('ticker'):
      ...
      clfW.index = pd.MultiIndex.from_tuples([(ticker, a) for a in clfW.index], names=['ticker','date'])
